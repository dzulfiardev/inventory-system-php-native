...
function fill_brand_list($connect, $categoryid)
{
    $query = "SELECT * FROM brand WHERE brand_status = 'active' AND category_id = '" . $categoryid . "' ORDER BY brand_name ASC ";
...
{
    $query = "SELECT user_name FROM user_detail WHERE user_id = '" . $user_id . "' 
    ";
    $statement = $connect->prepare($query);
    $statement->execute();
    $result = $statement->fetchAll();
...
function get_total_inventory_order_product($connect, $order_id)
{
    $query = "SELECT inventory_order_total FROM inventory_order WHERE inventory_order_id = '" . $order_id . "'";
    $statement = $connect->prepare($query);
    $statement->execute();
    $result = $statement->fetchAll();

    foreach ($result as $row) {
        return $row['inventory_order_total'];
    }
}
...
function avaible_product_quantity($connect, $product_id)
{
    $product_data = fetch_product_details($product_id, $connect);

    $query = "SELECT  inventory_order_product.quantity FROM inventory_order_product 
    ...
    $avaible_quantity = intval($product_data['quantity']) - intval($total);

    if ($avaible_quantity == 0) {
        $update_query = "UPDATE product SET product_status = 'inactive' 
    ...
    return $avaible_quantity;
}

function count_total_user($connect)
{
    $query = "SELECT * FROM user_detail WHERE user_status = 'active'";
    $statement = $connect->prepare($query);
    $statement->execute();
...
{
    $query = "SELECT sum(inventory_order_total) as total_order_value FROM inventory_order WHERE payment_status = 'cash' AND inventory_order_status='active'";
...
function get_user_wise_total_order($connect)
{
    ...
    FROM inventory_order INNER JOIN user_detail ON user_detail.user_id = inventory_order.user_id WHERE inventory_order.inventory_order_status = "active" GROUP BY inventory_order.user_id
    ';
    ...
    foreach ($result as $row) {
        $output .= '    
                    <tbody>
                        ...
                        <td>$ ' . $row["cash_order_total"] . '</td>
                        <td>$ ' . $row["credit_order_total"] . '</td>
                        </tr> 
                    </tbody>
            ';
